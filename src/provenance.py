"""Enforces the Stage 1 preservation invariant.

Stage 1 (extract_scenario.py) is the frozen, gold-verified semantic
...
completion (complete_parameters.py) — is allowed to FILL fields stage 1 left
null, but must never CHANGE a field stage 1 already set. This module checks
...
(Function/exception names below still say "agent1" — e.g. check_agent1_preserved,
Agent1FieldOverwritten — kept as-is; this is naming, not behavior.)

Only the semantic sections stage 1 actually produces are checked — the
...
don't exist in stage 1's output at all).
"""
